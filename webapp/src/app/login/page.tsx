...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        // Hybrid Auth Sync: Broadcast JWT to Chrome Extension
        ...
        router.push('/');
        router.refresh(); // Refresh layout to update auth state if needed
      } else {
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center">
      <div className="glass-card w-full max-w-md p-8 relative overflow-hidden group">
        ...
        <div className="absolute -bottom-20 -left-20 w-40 h-40 bg-accent/20 rounded-full blur-3xl group-hover:bg-accent/30 transition-all duration-700"></div>
        ...
        <div className="relative z-10">
          ...
          <p className="text-gray-400 text-center mb-8">Login to track your LeetCode progress</p>
          ...
          <p className="mt-8 text-center text-gray-400 text-sm">
            ...
            <Link href="/register" className="text-primary hover:text-primaryHover font-medium transition-colors">
              Create one
            </Link>
          </p>
        </div>
